...
n_layer = 8
dropout = 0.0
cutoff = 0.4 #STS-B similarity cutoff
k=1 #number of top schemas to retrieve
# ------------
...
async def generate_text(request: GenerateRequest):
    ...
    # Load trained parameters before generation
    try:
    ...
    try:
        ...
        schema="CREATE TABLE green_buildings (id INT, name VARCHAR(50), country VARCHAR(50), energy_consumption INT); INSERT INTO green_buildings (id, name, country, energy_consumption) VALUES (1, 'GreenHub', 'India', 1200), (2, 'EcoTower', 'China', 1500), (3, 'SolarVista', 'Japan', 1800);$"
        #schema = retrieve(request.prompt)[0]  # Uses RAG to fetch most relevant schema
        schema_new = schema
        context = torch.tensor(encode(request.prompt + schema), dtype=torch.long, device=device)
    except Exception as e:
    ...
    try:
        generated_ids = model.generate(context.to(device), max_new_tokens=max(request.max_new_tokens, 512))[0]
        full_text = decode(generated_ids.tolist())
        prompt_text = decode(generated_ids[:input_len].tolist())
        generated_text = decode(generated_ids[input_len:].tolist())

    except Exception as e:
        # return a 500 with the exception message for easier debugging
        raise HTTPException(status_code=500, detail=f"Generation failed: {e}")

    #HALLUCINATION FIX: Load reverse decoder parameters and regenerate
    try:
        load_path = project_root / f"script_trained_params_reversed_final.txt"
        load_model_params(str(load_path))
    except Exception as e:
        print(f"Warning: failed to load reverse decoder parameters: {e}")

    try:
        context = torch.tensor(encode(schema + generated_text), dtype=torch.long, device=device)
        #context = context.unsqueeze(0)
        context = torch.stack([context])
        context = torch.flip(context, dims=[1])
        input_len = context.size(1)
        generated_ids = model.generate(context.to(device), max_new_tokens=max(request.max_new_tokens, 512))[0]
        full_reverse_text = decode(generated_ids.tolist())
        prompt_reverse_text = decode(generated_ids[:input_len].tolist())
        generated_reverse_text = decode(generated_ids[input_len:].tolist()[::-1])  #reverse the generated text
    except Exception as e:
        # return a 500 with the exception message for easier debugging
        raise HTTPException(status_code=500, detail=f"Reverse Decoder Generation failed: {e}")

    # Compute cross-encoder similarity score
    try:
        cross_encoder = CrossEncoder("cross-encoder/stsb-roberta-base")
        score = cross_encoder.predict([(request.prompt, generated_reverse_text)])
        #print("Generated Text:", generated_text)
        print("Cross-encoder similarity score:", score[0])
        #print("Generated Reverse Text:", generated_reverse_text)
        if(score[0] < cutoff):
            generated_text = "I don't know"
    except Exception as e:
        print(f"Warning: failed to compute cross-encoder score: {e}")

    return GenerateResponse(generated_text=generated_text, schema_new=schema_new)
...
def load_all_pairs(path):
    ...
    for i in range(0, len(lines), 3):
        s1 = lines[i][:].lower()
        s2 = lines[i+1][:-1].lower()    # remove $
        t = lines[i+2][:-2].lower()  # remove ;;
        #print(f"Loaded pair:\n  SRC1: {s1}\n SRC2: {s2}\n TGT: {t}\n")
        chunks.append(s1)
        schema.append(s2)
...
def EmbeddingData(embedding_model):

    chunks, schema = load_all_pairs(input_path)
    ...
    return embeddings, schema


def save_embeddings_pt(embeddings, path):
    # Move to CPU for portability
    torch.save(embeddings.detach().cpu(), path)

def load_embeddings_pt(path, device="cpu"):
...
def retrieve(query):
    ...
    scores = F.cosine_similarity(
        query_embedding.unsqueeze(0), embeddings_new
    )

    top_k_indices = torch.topk(scores, k).indices.tolist()
    return [schema[i] for i in top_k_indices]


# Example: generate from the model
context = torch.zeros((1, 1), dtype=torch.long, device=device)
question = "What is the maximum energy consumption of green buildings?#"

sts_model = CrossEncoder(
...
#embeddings, schema=EmbeddingData(embedding_model=embedding_model)
#save_embeddings_pt(embeddings, "embeddings.pt")
embeddings_new, schema = load_embeddings_pt("embeddings.pt", device=device)

correct_schema=retrieve(question)
question += correct_schema[0] + "$"
#print(question)

context = torch.tensor(encode(question), dtype=torch.long, device=device)
context = torch.stack([context])
print(decode(model.generate(context, max_new_tokens=512)[0].tolist()))
