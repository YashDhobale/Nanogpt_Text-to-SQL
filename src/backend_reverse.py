...
text2 = "SELECT name, age FROM users WHERE age > 30;"
...
for iter in range(max_iters):
        ...
        if iter % eval_interval == 0 or iter == max_iters-1:
            ...
            print(decode(model.generate(context, max_new_tokens=100)[0].tolist()[::-1]))
...
#    save_path = project_root / f"script_trained_params_reversed.txt"
...
async def generate_text(request: GenerateRequest):
    ...
    try:
        generated_text = decode(model.generate(context, max_new_tokens=256)[0].tolist()[::-1])
...
context = torch.zeros((1, 1), dtype=torch.long, device=device)
text2 = "CREATE TABLE inspections (restaurant_id INT, score INT, area VARCHAR(255));INSERT INTO inspections (restaurant_id, score, area) VALUES (1, 95, 'urban'), (2, 85, 'urban'), (3, 90, 'suburban'), (4, 80, 'rural'), (5, 92, 'urban');$"
context = torch.tensor(encode(text2), dtype=torch.long, device=device)
context = torch.stack([context])
context = torch.flip(context, dims=[1])
print(decode(model.generate(context, max_new_tokens=512)[0].tolist()[::-1]))